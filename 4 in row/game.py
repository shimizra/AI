...
#Returns an empty board. The human plays first.
    ...
    for i in range(6): # 6 rows
        board=board+[7*[0]] #7 culomn
    return [board, 0.00001, HUMAN, 42] #42 moves (6*7)
...
# Assumes the move is legal.
    for r in range(6)[::-1]: #start from buttom  and find the first empty cell
        if s[0][r][c]==0:    #that mean its empty
...
# Reads, enforces legality and executes the user's move.
    ...
    while flag:
        ...
        if move<0 or move>=len(s[0][0]) or s[0][0][move]!=0:
            print("Ilegal move.")
        else:
            flag=False
            makeMove(s,move)
...
#the functoin wiil sort the states 
    ns=[]
    for c in range(len(s[0][0])):
            if s[0][0][c]==0:
                tmp=copy.deepcopy(s)
                makeMove(tmp,c)
                ns+=[tmp]
    return sorted(ns, key=lambda s: s[1], reverse=True)
